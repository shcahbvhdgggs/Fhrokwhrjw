...
if not TOKEN:
    raise SystemExit("Set NF_BOT_TOKEN env var first (do NOT hardcode token)")
...
COOLDOWN_429 = 15
LIVE_EVERY = 1
COOKIE_TIMEOUT = 30    # hard cap per cookie in seconds
EDIT_THROTTLE = 0.8    # seconds between telegram edits
...
ALL_PROFILES = IPHONES + LAPTOPS + ANDROIDS
ACCEPT_LANGS = ["ar-SA,ar;q=0.9,en;q=0.8", "ar-AE,ar;q=0.9,en;q=0.8",
...
def _check_worker(cd):
    if not cd.get("NetflixId"):
        return {"ok": False, "retries": 0, "tag": "no_id", "device": "?"}
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            if r.status_code in (429, 502, 503, 504):
                retries += 1
                time.sleep(COOLDOWN_429 + random.uniform(0, 8))
                continue
...
async def check_cookie_async(cd):
    try:
        ...
        log.warning("cookie check error: %s", e)
...
def p_menu():
    return (
        "🎬 <b>NETFLIX COOKIE CHECKER v10</b> 🍪\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
...
def p_stats():
    return (
        "📊 <b>BOT INFORMATION</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "🎭 <b>Fingerprints:</b> 8 profiles\n"
        "🔐 <b>TLS:</b> Chrome 126\n"
        "🌍 <b>Locale:</b> ar-SA / ar-AE / en-US / en-GB\n"
        "🧠 <b>Delay:</b> 0.35–1.4s jitter\n"
        "⏱️ <b>Cookie timeout:</b> 30s\n"
...
async def cb_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await q.message.reply_text(p_menu(), reply_markup=kb_main(), parse_mode="HTML")
    await q.answer()
